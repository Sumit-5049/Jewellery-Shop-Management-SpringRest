...
import com.jsm.dao.ReturnReqRepository;
...
import com.jsm.exception.NotPossibleException;
...
//output: String msg
	public Object addReturnReq(ReturnReq r, String email) throws DataNotFoundException, NotPossibleException, DuplicateException {
		if(bservice.exitById(r.getBill().getBillid())) {
	        ...
	        if(!dao.existsByBill(r.getBill())) {
	        	...
	        	log.error(env.getProperty("persentr")+r.getBill().getBillid());
	        	throw new DuplicateException(env.getProperty("persentr")+r.getBill().getBillid());
	        }
		}
